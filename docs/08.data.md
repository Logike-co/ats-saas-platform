...
erDiagram
    ...
    CANDIDATE ||--o{ CANDIDATE_FILE : stores
...
### `candidate` (directorio / talent pool)

Candidato **por tenant**; puede existir **sin** `application`. Un mismo candidato puede tener varias candidaturas a distintas vacantes.

- `id` (uuid, pk)
- `tenant_id` (uuid, fk)
- `first_name` (varchar)
- `last_name` (varchar)
- `email` (varchar, **unico por `(tenant_id, email_normalizado)`**)
- `phone` (varchar, nullable)
- `address` (text, nullable)
- `education` (text, nullable) — perfil enriquecido; evolucionable a estructura tipada mas adelante
- `work_experience` (text, nullable) — igual que `education`
- `created_at`, `updated_at`, `deleted_at`

**Nota de presentacion:** la UI puede mostrar `first_name` + `last_name` como nombre completo; se puede derivar un campo calculado en lectura si se desea.

### `candidate_file`

Adjuntos del candidato en directorio (p. ej. **CV**), sin requerir candidatura.

- `id` (uuid, pk)
- `tenant_id` (uuid, fk)
- `candidate_id` (uuid, fk)
- `file_type` (enum: cv, portfolio, certificate, other)
- `file_url` o `storage_path` (text) — ruta en volumen / disco VPS en fase inicial
- `mime_type` (varchar), `size_bytes` (bigint)
- `created_at`

**Politica v1 (bajo coste y esfuerzo):**

- Formatos permitidos: **PDF**, **DOCX** (validar MIME ademas de extension).
- Tamano maximo recomendado: **5–10 MB** (ajustar en configuracion).
- Almacenamiento: **disco en el VPS** con jerarquia por `tenant_id` / `candidate_id`, sin servicio de objeto externo en MVP.
- No se exige antivirus de terceros en v1; se recomienda validacion estricta de tipo y tamano.
...
- Un `candidate` puede existir solo en **directorio** o tener **cero o muchas** `applications` a distintas `jobs`.
- Un `candidate` puede tener **cero o muchos** `candidate_file` (CV y otros).
...
- Unicidad recomendada:
  - `candidate(tenant_id, email_normalizado)` unico
...
- Archivos (CV y anexos de **candidato** y de **candidatura**) fuera de la BD: **filesystem en VPS** en fase inicial, guardando ruta o URL y metadatos en tablas `candidate_file` / `application_file`.
- Evolucion futura: almacenamiento objeto (S3 compatible) cuando volumen o redundancia lo exijan.
