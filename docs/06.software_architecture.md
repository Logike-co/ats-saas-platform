...
El enfoque es **modular pragmatico**: organizacion por **feature** y profundidad de capas donde aporte valor, sin obligar a la misma estructura en cada modulo. Eso **no** relaja **unica responsabilidad (SRP)** ni **interfaz segregada (ISP)** en casos de uso y puertos; ver `docs/05.design_principles.md` y el detalle operativo en `docs/07.code_and_technical_design.md`.
...
Las flechas muestran **dependencias entre capas logicas** dentro del monolito. Cada **modulo de dominio** (`jobs`, `applications`, `candidates`, etc.) **repite** este reparto (API, aplicacion, dominio, infraestructura); no existe una capa de aplicacion global que “importe” a los modulos como librerias — eso evitaba el diagrama anterior (`Application --> Jobs Module`), que prestaba a confusion.

```mermaid
graph TD
    subgraph Nest["NestJS Monolito Modular"]
        GW["API: controllers, DTOs, validacion"]
        APP["Aplicacion: casos de uso, puertos in/out"]
        DOM["Dominio: entidades y reglas"]
        INF["Infraestructura: Prisma, colas, adaptadores"]
    end
